...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--limit', type=int, default=151, help='Number of Pokemon (default: 151)')
        parser.add_argument('--delay', type=float, default=0.1, help='Pause between requests (default: 0.1s)')
    ...
    def handle(self, *args, **options):
        ...
        logger.info(f"🚀 Starting synchronization of {limit} Pokemon at {datetime.now().strftime('%H:%M:%S')}")
        ...
        for i, url in enumerate(pokemon_urls, 1):
            ...
            if i % max(1, total_pokemon // 20) == 0 or i == total_pokemon:  # Every 5%
                logger.info(f"📊 Progress: {current_percent:.1f}% ({i}/{total_pokemon}) - Saved: {saved_count}")
        ...
        logger.info(f"📊 Saved: {saved_count}, Errors: {error_count}, API calls: {api_calls}")
        logger.info(f"🚀 Speed: {api_calls/total_time:.1f} calls/s, Success rate: {success_rate:.1f}%")
        ...
        try:
            ...
            for attempt in range(3):
                try:
                    ...
                    if attempt == 2:
                        logger.error(f'Final error after 3 attempts for Pokemon #{pokemon_id}: {e}')
                        return None
                    time.sleep(1)
