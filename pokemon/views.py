...
from .services.pokeapi_service import (
    get_pokemon_from_db,
    fetch_from_api_and_save,
    has_all_pokemon_in_db,
    create_response_from_db,
)
import logging
...
def pokemon(request):
    try:
        ...
        existing_pokemon = get_pokemon_from_db(
            pokemon_range_start, pokemon_range_end
        )
        expected_count = pokemon_range_end - pokemon_range_start

        if has_all_pokemon_in_db(existing_pokemon, expected_count):
            page_data = create_response_from_db(
                existing_pokemon, page_number, per_page
            )
        else:
            page_data = fetch_from_api_and_save(
                pokemon_range_start, pokemon_range_end, page_number, per_page
            )
