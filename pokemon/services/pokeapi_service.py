...
def get_pokemon_from_db(
    pokemon_range_start: int, pokemon_range_end: int
) -> List[Pokemon]:
    return list(
        Pokemon.objects.filter(
            api_id__gte=pokemon_range_start, api_id__lt=pokemon_range_end
        ).prefetch_related("types__type", "abilities")
    )


def has_all_pokemon_in_db(
    existing_pokemon: List[Pokemon], expected_count: int
) -> bool:
    return len(existing_pokemon) == expected_count


def create_response_from_db(
    existing_pokemon: List[Pokemon], page: int, per_page: int
) -> Dict[str, Any]:
    pokemon_list = []
...
def fetch_from_api_and_save(
    pokemon_range_start: int, pokemon_range_end: int, page: int, per_page: int
) -> Dict[str, Any]:
    ...
    try:
        with requests.Session() as session:
            ...
            for pokemon_entry in pokemon_list_data["results"]:
                ...
                display_pokemon_list.append(
                    format_pokemon_for_display(pokemon_detail_data)
                )
...
def format_pokemon_for_display(
    pokemon_detail_data: Dict[str, Any],
) -> Dict[str, Any]:
    return {
        ...
        "types": [
            type_info["type"]["name"] for type_info in pokemon_detail_data["types"]
        ],
        "abilities": [
...
def create_page_response(
    pokemon_list: List[Dict[str, Any]], page: int, per_page: int, data_source: str
) -> Dict[str, Any]:
